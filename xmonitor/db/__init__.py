...
from xmonitor.api.v2.model.metadef_property_type import PropertyType
from xmonitor.common import crypt
from xmonitor.common import exception
from xmonitor.common.glare import serialization
from xmonitor.common import location_strategy
import xmonitor.domain
import xmonitor.domain.proxy
from xmonitor import glare as ga
from xmonitor.i18n import _

CONF = cfg.CONF
CONF.import_opt('image_size_cap', 'xmonitor.common.config')
CONF.import_opt('metadata_encryption_key', 'xmonitor.common.config')
...
class ArtifactRepo(object):
    ...
    def _format_artifact_from_db(self, db_artifact):
        ...
        return xmonitor.domain.Artifact(**kwargs)
...
class ImageMemberRepo(object):
    ...
    def _format_image_member_from_db(self, db_image_member):
        return xmonitor.domain.ImageMembership(
...
class TaskRepo(object):
    ...
    def _format_task_stub_from_db(self, db_task):
        return xmonitor.domain.TaskStub(
            task_id=db_task['id'],
            task_type=db_task['type'],
            status=db_task['status'],
            owner=db_task['owner'],
...
class MetadefNamespaceRepo(object):
    ...
    def _format_namespace_from_db(self, namespace_obj):
        return xmonitor.domain.MetadefNamespace(
...
class MetadefObjectRepo(object):
    ...
    def _format_metadef_object_from_db(self, metadata_object,
        ...
        return xmonitor.domain.MetadefObject(
...
class MetadefResourceTypeRepo(object):
    ...
    def _format_resource_type_from_db(self, resource_type, namespace):
        return xmonitor.domain.MetadefResourceType(
            namespace=namespace,
...
class MetadefPropertyRepo(object):
    ...
    def _format_metadef_property_from_db(
        ...
        return xmonitor.domain.MetadefProperty(
            namespace=namespace_entity,
...
class MetadefTagRepo(object):
    ...
    def _format_metadef_tag_from_db(self, metadata_tag,
                                    namespace_entity):
        return xmonitor.domain.MetadefTag(
            namespace=namespace_entity,
