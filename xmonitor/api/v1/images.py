...
from xmonitor.api import common
from xmonitor.api import policy
import xmonitor.api.v1
from xmonitor.api.v1 import controller
from xmonitor.api.v1 import filters
...
from xmonitor.i18n import _, _LE, _LI, _LW
from xmonitor import notifier
import xmonitor.registry.client.v1.api as registry

LOG = logging.getLogger(__name__)
SUPPORTED_PARAMS = xmonitor.api.v1.SUPPORTED_PARAMS
SUPPORTED_FILTERS = xmonitor.api.v1.SUPPORTED_FILTERS
ACTIVE_IMMUTABLE = xmonitor.api.v1.ACTIVE_IMMUTABLE
IMMUTABLE = xmonitor.api.v1.IMMUTABLE

CONF = cfg.CONF
CONF.import_opt('disk_formats', 'xmonitor.common.config', group='image_format')
CONF.import_opt('container_formats', 'xmonitor.common.config',
                group='image_format')
CONF.import_opt('image_property_quota', 'xmonitor.common.config')


def _validate_time(req, values):
    """Validates time formats for updated_at, created_at and deleted_at.
    'strftime' only allows values after 1900 in xmonitor v1 so this is enforced
...
class Controller(controller.BaseController):
    ...
    def _get_size(self, context, image_meta, location):
        ...
        try:
            ...
            # do with xmonitor. If log.exception is used here, in that case the
    ...
    def create(self, req, image_meta, image_data):
        ...
        2. If the image data exists somewhere else, you can upload indirectly
           from the external source using the x-xmonitor-api-copy-from header.
           ...
           xmonitor image store.  State transitions are as for option #1.
        ...
        3. If the image data exists somewhere else, you can reference the
           ...
           to the configured xmonitor image store.
    ...
    def update(self, req, id, image_meta, image_data):
        ...
        purge_props = req.headers.get('x-xmonitor-registry-purge-props', True)
