...
import xmonitor.api
import xmonitor.api.common
from xmonitor.api.v1 import router
from xmonitor.api.v1 import upload_utils
import xmonitor.common.config
from xmonitor.common import exception
from xmonitor.common import timeutils
import xmonitor.context
from xmonitor.db.sqlalchemy import api as db_api
from xmonitor.db.sqlalchemy import models as db_models
import xmonitor.registry.client.v1.api as registry
from xmonitor.tests.unit import base
import xmonitor.tests.unit.utils as unit_test_utils
...
class TestGlanceAPI(base.IsolatedUnitTest):
        ...
        self.FIXTURES = [
            ...
            {'id': UUID3,
             ...
             'locations': [{'url': "file:///%s/%s" % (self.test_dir, UUID1),
                            'metadata': {}, 'status': 'active'}],
             'properties': {}}]
        self.context = xmonitor.context.RequestContext(is_admin=True)
        db_api.get_engine()
    ...
    def test_bad_time_create_low_year(self):
        # 'strftime' only allows values after 1900 in xmonitor v1
        fixture_headers = {'x-image-meta-store': 'file',
                           'x-image-meta-disk-format': 'vhd',
                           'x-image-meta-container-format': 'ovf',
    ...
    @mock.patch.object(xmonitor.api.v1.images.Controller, '_external_source')
    ...
    def test_add_copy_from_with_nonempty_body(self):
        ...
        fixture_headers = {'x-image-meta-store': 'file',
                           'x-image-meta-disk-format': 'vhd',
                           'x-xmonitor-api-copy-from': 'http://0.0.0.0:1/c.ovf',
                           'x-image-meta-container-format': 'ovf',
                           'x-image-meta-name': 'fake image #F'}
    ...
    def test_add_copy_from_with_restricted_sources(self):
        ...
        for schema in schemas:
            ...
            for k, v in six.iteritems(header_template):
                req.headers[k] = v
            req.headers['x-xmonitor-api-copy-from'] = schema
            res = req.get_response(self.api)
            self.assertEqual(400, res.status_int)
    ...
    def test_update_copy_from_upload_image_unauthorized(self):
        ...
        req.method = 'PUT'
        req.headers['Content-Type'] = 'application/octet-stream'
        req.headers['x-xmonitor-api-copy-from'] = self._http_loc_url('/i.ovf')
        res = req.get_response(self.api)
        self.assertEqual(403, res.status_int)
    ...
    def _check_delete_during_image_upload(self, is_admin=False):
        ...
        self.stubs.Set(xmonitor.api.v1.upload_utils, 'initiate_deletion',
    ...
    def test_disable_purge_props(self):
        """
        Test the special x-xmonitor-registry-purge-props header controls
        ...
        # passing the x-xmonitor-registry-purge-props header and
    ...
    def test_download_service_unavailable(self):
        ...
        image_controller = xmonitor.api.v1.images.Controller()
...
class TestImageSerializer(base.IsolatedUnitTest):
    def setUp(self):
        ...
        self.context = xmonitor.context.RequestContext(
            is_admin=True,
            user=self.receiving_user,
            tenant=self.receiving_tenant)
        self.serializer = xmonitor.api.v1.images.ImageSerializer()
    ...
    def test_meta(self):
        exp_headers = {'x-image-meta-id': UUID2,
                       'x-image-meta-location': 'file:///tmp/xmonitor-tests/2',
    ...
    def test_meta_utf8(self):
        ...
        FIXTURE = {
            'image_meta': {
                ...
                'location': u"file:///tmp/xmonitor-tests/2",
                'properties': {
    ...
    def test_image_send_notification(self):
        ...
        xmonitor.api.common.image_send_notification(19, 19, image_meta, req,
    ...
    def test_image_send_notification_error(self):
        ...
        xmonitor.api.common.image_send_notification(17, 19, image_meta, req,
                                                    self.serializer.notifier)
    ...
    def test_redact_location(self):
        ...
        tmp_image_meta = xmonitor.api.v1.images.redact_loc(image_meta)
...
class TestFilterValidator(base.IsolatedUnitTest):
    def test_filter_validator(self):
        self.assertFalse(xmonitor.api.v1.filters.validate('size_max', -1))
        self.assertTrue(xmonitor.api.v1.filters.validate('size_max', 1))
        self.assertTrue(xmonitor.api.v1.filters.validate('protected', 'True'))
        self.assertTrue(xmonitor.api.v1.filters.validate('protected', 'FALSE'))
        self.assertFalse(xmonitor.api.v1.filters.validate('protected', '-1'))
...
class TestAPIPropertyQuotas(base.IsolatedUnitTest):
    ...
    def test_update_image_with_too_many_properties_without_purge_props(self):
        ...
        another_request = unit_test_utils.get_fake_request(
            path='/images/%s' % image_id, method='PUT')
        headers = {'x-auth-token': 'user:tenant:joe_soap',
                   'x-xmonitor-registry-purge-props': 'False',
                   'x-image-meta-property-x_all_permitted': '1'}
        for k, v in six.iteritems(headers):
            another_request.headers[k] = v
