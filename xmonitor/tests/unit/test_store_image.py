...
from xmonitor.tests.unit import base as unit_test_base
from xmonitor.tests.unit import utils as unit_test_utils
...
class FakeMemberRepo(object):
    def __init__(self, image, tenants=None):
        self.image = image
        self.factory = xmonitor.domain.ImageMemberFactory()
...
class TestStoreImage(utils.BaseTestCase):
    ...
    def test_image_delete(self):
        image = xmonitor.location.ImageProxy(self.image_stub, {},
    ...
    def test_image_get_data_from_second_location(self):
        ...
        image1 = xmonitor.location.ImageProxy(self.image_stub, {},
                                              self.store_api, self.store_utils)
        self.assertEqual('XXX', image1.get_data())
        # Multiple location support
        context = xmonitor.context.RequestContext(user=USER1)
    ...
    def test_image_set_data(self):
        ...
        image = xmonitor.location.ImageProxy(image_stub, context,
                                             self.store_api, self.store_utils)
    ...
    @mock.patch('xmonitor.location.LOG')
    ...
    def test_image_change_adding_invalid_location_uri(self):
        ...
        image1 = xmonitor.location.ImageProxy(image_stub1, context,
                                              self.store_api, self.store_utils)
    ...
    def test_image_change_adding_invalid_location_metadata(self):
        ...
        image2 = xmonitor.location.ImageProxy(image_stub2, context,
                                              self.store_api, self.store_utils)
    ...
    def test_image_change_adding_locations(self):
        ...
        image3 = xmonitor.location.ImageProxy(image_stub3, context,
                                              self.store_api, self.store_utils)
...
class TestStoreImageRepo(utils.BaseTestCase):
    def setUp(self):
        ...
        self.image = xmonitor.location.ImageProxy(self.image_stub, {},
                                                  self.store_api, store_utils)
        self.image_repo_stub = ImageRepoStub()
        self.image_repo = xmonitor.location.ImageRepoProxy(self.image_repo_stub,
                                                           {}, self.store_api,
                                                           store_utils)
        patcher = mock.patch("xmonitor.location._get_member_repo_for_store",
                             self.get_fake_member_repo)
        patcher.start()
        self.addCleanup(patcher.stop)
        self.fake_member_repo = FakeMemberRepo(self.image, [TENANT1, TENANT2])
        self.image_member_repo = xmonitor.location.ImageMemberRepoProxy(
    ...
    def test_member_addition_updates_acls(self):
        self.image_stub.locations = [{'url': 'glug', 'metadata': {},
                                      'status': 'active'}]
        self.image_stub.visibility = 'private'
        membership = xmonitor.domain.ImageMembership(
...
class TestImageFactory(unit_test_base.StoreClearingUnitTest):
    ...
    def setUp(self):
        ...
        store_utils = unit_test_utils.FakeStoreUtils(store_api)
        self.image_factory = xmonitor.location.ImageFactoryProxy(
            ImageFactoryStub(),
            xmonitor.context.RequestContext(user=USER1),
            store_api,
            store_utils)
