...
class TestPolicyEnforcer(base.IsolatedUnitTest):
    def test_policy_file_default_rules_default_location(self):
        enforcer = xmonitor.api.policy.Enforcer()

        context = xmonitor.context.RequestContext(roles=[])
...
class TestPolicyEnforcerNoFile(base.IsolatedUnitTest):
    def test_policy_file_specified_but_not_found(self):
        ...
        admin_context = xmonitor.context.RequestContext(roles=['admin'])
...
class TestImagePolicy(test_utils.BaseTestCase):
    ...
    def test_publicize_image_not_allowed(self):
        ...
        image = xmonitor.api.policy.ImageProxy(self.image_stub, {}, self.policy)
        self.assertRaises(exception.Forbidden,
    ...
    def test_get_image_not_allowed(self):
        ...
        with mock.patch.object(xmonitor.api.policy, 'ImageTarget') as target:
            target.return_value = image_target
            image_repo = xmonitor.api.policy.ImageRepoProxy(self.image_repo_stub,
                                                            {}, self.policy)
    ...
    def test_get_image_allowed(self):
        ...
        self.assertIsInstance(output, xmonitor.api.policy.ImageProxy)
    ...
    def test_get_images_allowed(self):
        ...
        for i, image in enumerate(images):
            self.assertIsInstance(image, xmonitor.api.policy.ImageProxy)
    ...
    def test_new_image_visibility(self):
        self.policy.enforce.side_effect = exception.Forbidden
        image_factory = xmonitor.api.policy.ImageFactoryProxy(
    ...
    def test_image_get_data_policy_enforced_with_target(self):
        ...
        with mock.patch('xmonitor.api.policy.ImageTarget'):
            image = xmonitor.api.policy.ImageProxy(image_stub, {}, self.policy)
        target = image.target
        self.policy.enforce.side_effect = exception.Forbidden
...
class TestMemberPolicy(test_utils.BaseTestCase):
    def setUp(self):
        ...
        self.image_stub = ImageStub(UUID1)
        image = xmonitor.api.policy.ImageProxy(self.image_stub, {}, self.policy)
        self.member_repo = xmonitor.api.policy.ImageMemberRepoProxy(
...
class TestTaskPolicy(test_utils.BaseTestCase):
    ...
    def test_get_task_not_allowed(self):
        ...
        task_repo = xmonitor.api.policy.TaskRepoProxy(
            self.task_repo_stub,
            {},
            self.policy
        )
    ...
    def test_get_task_allowed(self):
        ...
        self.assertIsInstance(task, xmonitor.api.policy.TaskProxy)
        self.assertEqual('task_from_get', task.task)

    def test_get_tasks_not_allowed(self):
        rules = {"get_tasks": False}
        self.policy.set_rules(rules)
        task_repo = xmonitor.api.policy.TaskStubRepoProxy(
            self.task_repo_stub,
            {},
            self.policy
        )
    ...
    def test_get_tasks_allowed(self):
        ...
        for i, task in enumerate(tasks):
            self.assertIsInstance(task, xmonitor.api.policy.TaskStubProxy)
    ...
    def test_add_task_not_allowed(self):
        ...
        task = xmonitor.api.policy.TaskProxy(self.task_stub, {}, self.policy)
...
class TestContextPolicyEnforcer(base.IsolatedUnitTest):
    def _do_test_policy_influence_context_admin(self,
        ...
        context = xmonitor.context.RequestContext(roles=[context_role],
