...
import xmonitor.db
from xmonitor.db.sqlalchemy import api
...
class TestDbUtilities(test_utils.BaseTestCase):
    ...
    def test_get_api_calls_configure_if_present(self, import_module):
        ...
        self.assertEqual(xmonitor.db.get_api(), self.api)
...
class TestImageMemberRepo(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.context = xmonitor.context.RequestContext(
            user=USER1, tenant=TENANT1)
        self.image_repo = xmonitor.db.ImageRepo(self.context, self.db)
        self.image_member_factory = xmonitor.domain.ImageMemberFactory()
        self._create_images()
        self._create_image_members()
        image = self.image_repo.get(UUID1)
        self.image_member_repo = xmonitor.db.ImageMemberRepo(self.context,
                                                             self.db, image)
    ...
    def test_list_no_members(self):
        image = self.image_repo.get(UUID2)
        self.image_member_repo_uuid2 = xmonitor.db.ImageMemberRepo(
    ...
    def test_remove_image_member_does_not_exist(self):
        fake_uuid = str(uuid.uuid4())
        image = self.image_repo.get(UUID2)
        fake_member = xmonitor.domain.ImageMemberFactory().new_image_member(
            image, TENANT4)
