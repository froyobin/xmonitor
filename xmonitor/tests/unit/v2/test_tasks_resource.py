...
def _domain_fixture(task_id, **kwargs):
    ...
    task = xmonitor.domain.Task(**task_properties)
    return task

CONF = cfg.CONF
CONF.import_opt('task_time_to_live', 'xmonitor.common.config', group='task')
...
class TestTasksController(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self._create_tasks()
        self.controller = xmonitor.api.v2.tasks.TasksController(self.db,
                                                                self.policy,
                                                                self.notifier,
                                                                self.store)
    ...
    @mock.patch.object(xmonitor.gateway.Gateway, 'get_task_factory')
    @mock.patch.object(xmonitor.gateway.Gateway, 'get_task_executor_factory')
    @mock.patch.object(xmonitor.gateway.Gateway, 'get_task_repo')
    ...
    @mock.patch('xmonitor.common.scripts.utils.get_image_data_iter')
    @mock.patch('xmonitor.common.scripts.utils.validate_location_uri')
...
class TestTasksDeserializer(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.deserializer = xmonitor.api.v2.tasks.RequestDeserializer()
...
class TestTasksSerializer(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.serializer = xmonitor.api.v2.tasks.ResponseSerializer()
        self.fixtures = [
