...
from xmonitor.cmd import manage
from xmonitor.db import migration as db_migration
from xmonitor.db.sqlalchemy import api as db_api
from xmonitor.db.sqlalchemy import metadata as db_metadata
...
class TestManageBase(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        patcher = mock.patch('xmonitor.db.sqlalchemy.api.get_engine')
        patcher.start()
...
class TestLegacyManage(TestManageBase):
    ...
    def test_legacy_db_sync(self, db_sync):
        self._main_test_helper(['xmonitor.cmd.manage', 'db_sync'],
                               migration.db_sync,
                               db_api.get_engine(),
    ...
    def test_legacy_db_sync_version(self, db_sync):
        self._main_test_helper(['xmonitor.cmd.manage', 'db_sync', '20'],
                               migration.db_sync,
                               db_api.get_engine(),
    ...
    def test_legacy_db_upgrade_version(self, db_sync):
        self._main_test_helper(['xmonitor.cmd.manage', 'db_upgrade', '20'],
                               migration.db_sync,
                               db_api.get_engine(),
                               db_migration.MIGRATE_REPO_PATH, '20')
    ...
    def test_db_metadefs_export(self):
        ...
        self._main_test_helper(['xmonitor.cmd.manage', 'db_export_metadefs'],
...
class TestManage(TestManageBase):
    ...
    def test_db_version(self, db_version):
        with mock.patch('sys.stdout', new_callable=StringIO):
            self._main_test_helper(['xmonitor.cmd.manage', 'db', 'version'],
                                   migration.db_version,
                                   db_api.get_engine(),
                                   db_migration.MIGRATE_REPO_PATH, 0)
    ...
    def test_db_sync(self, db_sync):
        self._main_test_helper(['xmonitor.cmd.manage', 'db', 'sync'],
                               migration.db_sync,
                               db_api.get_engine(),
                               db_migration.MIGRATE_REPO_PATH, None)
    ...
    def test_db_upgrade(self, db_sync):
        self._main_test_helper(['xmonitor.cmd.manage', 'db', 'upgrade'],
                               migration.db_sync,
                               db_api.get_engine(),
                               db_migration.MIGRATE_REPO_PATH, None)
    ...
    def test_db_version_control(self, db_version_control):
        self._main_test_helper(['xmonitor.cmd.manage', 'db', 'version_control'],
                               migration.db_version_control,
                               db_api.get_engine(),
                               db_migration.MIGRATE_REPO_PATH, None)
    ...
    def test_db_sync_version(self, db_sync):
        self._main_test_helper(['xmonitor.cmd.manage', 'db', 'sync', '20'],
                               migration.db_sync,
                               db_api.get_engine(),
                               db_migration.MIGRATE_REPO_PATH, '20')
    ...
    def test_db_upgrade_version(self, db_sync):
        self._main_test_helper(['xmonitor.cmd.manage', 'db', 'upgrade', '20'],
                               migration.db_sync,
                               db_api.get_engine(),
                               db_migration.MIGRATE_REPO_PATH, '20')
    ...
    def test_db_downgrade_version(self, db_sync):
        self._main_test_helper(['xmonitor.cmd.manage', 'db', 'downgrade', '20'],
                               migration.db_sync,
                               db_api.get_engine(),
                               db_migration.MIGRATE_REPO_PATH, '20')
    ...
    def test_db_metadefs_unload(self):
        db_metadata.db_unload_metadefs = mock.Mock()
        self._main_test_helper(['xmonitor.cmd.manage', 'db', 'unload_metadefs'],
                               db_metadata.db_unload_metadefs,
                               db_api.get_engine())
