...
class TestImageQuota(test_utils.BaseTestCase):
    ...
    def test_quota_exceeded_no_size(self):
        ...
        with patch.object(xmonitor.api.common, 'get_remaining_quota',
                          return_value=0):
...
class TestImagePropertyQuotas(test_utils.BaseTestCase):
    def setUp(self):
        ...
        self.image = xmonitor.quota.ImageProxy(self.base_image,
                                               mock.Mock(),
                                               mock.Mock(),
                                               mock.Mock())
        ...
        self.image_repo_proxy = xmonitor.quota.ImageRepoProxy(
...
class TestQuotaImageTagsProxy(test_utils.BaseTestCase):
    ...
    def test_add(self):
        proxy = xmonitor.quota.QuotaImageTagsProxy(set([]))
        proxy.add('foo')
    ...
    def test_contains(self):
        proxy = xmonitor.quota.QuotaImageTagsProxy(set(['foo']))
        self.assertIn('foo', proxy)
    ...
    def test_iter(self):
        ...
        proxy = xmonitor.quota.QuotaImageTagsProxy(items.copy())
