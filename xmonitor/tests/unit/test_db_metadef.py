...
class TestMetadefRepo(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.context = xmonitor.context.RequestContext(user=USER1,
                                                       tenant=TENANT1)
        self.namespace_repo = xmonitor.db.MetadefNamespaceRepo(self.context,
                                                               self.db)
        self.property_repo = xmonitor.db.MetadefPropertyRepo(self.context,
                                                             self.db)
        self.object_repo = xmonitor.db.MetadefObjectRepo(self.context,
                                                         self.db)
        self.tag_repo = xmonitor.db.MetadefTagRepo(self.context,
                                                   self.db)
