...
from xmonitor.tests import utils
...
class ImageStub(xmonitor.domain.Image):
...
class TaskStub(xmonitor.domain.TaskStub):
    def run(self, executor):
        pass


class Task(xmonitor.domain.Task):
...
class TestNotifier(utils.BaseTestCase):
    ...
    def test_set_defaults(self, mock_set_trans_defaults):
        ...
        mock_set_trans_defaults.assert_called_with('xmonitor')
...
class TestImageNotifications(utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.context = xmonitor.context.RequestContext(tenant=TENANT2,
                                                       user=USER1)
        self.image_repo_stub = ImageRepoStub()
        self.notifier = unit_test_utils.FakeNotifier()
        self.image_repo_proxy = xmonitor.notifier.ImageRepoProxy(
            self.image_repo_stub, self.context, self.notifier)
        self.image_proxy = xmonitor.notifier.ImageProxy(
    ...
    def test_image_get(self):
        ...
        self.assertIsInstance(image, xmonitor.notifier.ImageProxy)
        self.assertEqual('image_from_get', image.repo)

    def test_image_list(self):
        images = self.image_repo_proxy.list()
        self.assertIsInstance(images[0], xmonitor.notifier.ImageProxy)
...
class TestImageMemberNotifications(utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.image = ImageStub(
            ...
            min_disk=10, checksum='ca425b88f047ce8ec45ee90e813ada91',
            locations=['http://127.0.0.1'])
        self.image_member = xmonitor.domain.ImageMembership(
        ...
        self.image_member_repo_proxy = xmonitor.notifier.ImageMemberRepoProxy(
            self.image_member_repo_stub, self.image,
            self.context, self.notifier)
        self.image_member_proxy = xmonitor.notifier.ImageMemberProxy(
    ...
    def test_image_member_get(self):
        ...
        self.assertIsInstance(image_member, xmonitor.notifier.ImageMemberProxy)
    ...
    def test_image_member_list(self):
        ...
        self.assertIsInstance(image_members[0],
                              xmonitor.notifier.ImageMemberProxy)
...
class TestTaskNotifications(utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.notifier = unit_test_utils.FakeNotifier()
        self.task_repo_proxy = xmonitor.notifier.TaskRepoProxy(
            self.task_repo_stub,
            self.context,
            self.notifier
        )
        self.task_proxy = xmonitor.notifier.TaskProxy(
            self.task,
            self.context,
            self.notifier
        )
        self.task_stub_proxy = xmonitor.notifier.TaskStubProxy(
            self.task_stub,
            self.context,
            self.notifier
        )
    ...
    def test_task_run_notification(self):
        with mock.patch('xmonitor.async.TaskExecutor') as mock_executor:
