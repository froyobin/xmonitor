...
from xmonitor import tests as glance_tests
...
class Server(object):
    ...
    def create_database(self):
        ...
        if self.needs_database:
            conf_dir = os.path.join(self.test_dir, 'etc')
            utils.safe_mkdirs(conf_dir)
            conf_filepath = os.path.join(conf_dir, 'xmonitor-manage.conf')
...
class ApiServer(Server):
    ...
    def __init__(self, test_dir, port, policy_file, delayed_delete=False,
        ...
        self.server_module = 'xmonitor.cmd.%s' % self.server_name
...
"""
        self.paste_conf_base = """[pipeline:xmonitor-api]
pipeline =
    cors
    healthcheck
    versionnegotiation
...
[pipeline:xmonitor-api-fakeauth]
pipeline = cors healthcheck versionnegotiation gzip fakeauth context rootapp

[pipeline:xmonitor-api-noauth]
pipeline = cors healthcheck versionnegotiation gzip context rootapp

[composite:rootapp]
paste.composite_factory = xmonitor.api:root_app_factory
/: apiversions
/v1: apiv1app
/v2: apiv2app

[app:apiversions]
paste.app_factory = xmonitor.api.versions:create_resource

[app:apiv1app]
paste.app_factory = xmonitor.api.v1.router:API.factory

[app:apiv2app]
paste.app_factory = xmonitor.api.v2.router:API.factory
...
paste.filter_factory =
 xmonitor.api.middleware.version_negotiation:VersionNegotiationFilter.factory

[filter:gzip]
paste.filter_factory = xmonitor.api.middleware.gzip:GzipMiddleware.factory

[filter:cache]
paste.filter_factory = xmonitor.api.middleware.cache:CacheFilter.factory

[filter:cache_manage]
paste.filter_factory =
 xmonitor.api.middleware.cache_manage:CacheManageFilter.factory

[filter:context]
paste.filter_factory = xmonitor.api.middleware.context:ContextMiddleware.factory

[filter:unauthenticated-context]
paste.filter_factory =
 xmonitor.api.middleware.context:UnauthenticatedContextMiddleware.factory

[filter:fakeauth]
paste.filter_factory = xmonitor.tests.utils:FakeAuthMiddleware.factory
...
flavor = %(deployment_flavor)s
"""
        self.paste_conf_base = """[pipeline:xmonitor-registry]
pipeline = healthcheck unauthenticated-context registryapp

[pipeline:xmonitor-registry-fakeauth]
pipeline = healthcheck fakeauth context registryapp

[pipeline:xmonitor-registry-trusted-auth]
pipeline = healthcheck context registryapp

[app:registryapp]
paste.app_factory = xmonitor.registry.api:API.factory
...
class FunctionalTest(test_utils.BaseTestCase):
    ...
    def start_servers(self, **kwargs):
        """
        Starts the API and Registry servers (xmonitor-control api start
        & xmonitor-control registry start) on unused ports.  xmonitor-control
    ...
    def stop_server(self, server, name):
        ...
        xmonitor-control stop method to gracefully shut the server down.
    ...
    def stop_servers(self):
        ...
        the xmonitor-control stop method to gracefully shut the server down.
    ...
    def copy_data_file(self, file_name, dst_dir):
        src_file_name = os.path.join('xmonitor/tests/etc', file_name)
