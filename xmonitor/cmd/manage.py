...
# FIXME(sirp): When we have xmonitor-admin we can consider merging this into it
# Perhaps for consistency with Nova, we would then rename xmonitor-admin ->
# xmonitor-manage (or the other way around)
...
from xmonitor.db.sqlalchemy import metadata
...
class DbCommands(object):
    ...
    def purge(self, age_in_days=30, max_rows=100):
        """Purge deleted rows older than a given age from xmonitor tables."""
        try:
