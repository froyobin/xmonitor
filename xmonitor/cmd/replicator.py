...
# If ../xmonitor/__init__.py exists, add ../ to Python search path, so that
...
COMMANDS = """Commands:
    ...
    compare         What is missing from the slave xmonitor?
    dump            Dump the contents of a xmonitor instance to local disk.
    livecopy        Load the contents of one xmonitor instance into another.
    load            Load the contents of a local directory into xmonitor.
    size            Determine the size of a xmonitor instance if dumped to disk.
"""
...
class ImageService(object):
    def __init__(self, conn, auth_token):
        ...
        conn: a http_client.HTTPConnection to the xmonitor server
    ...
    def get_image(self, image_uuid):
        """Fetch image data from xmonitor.
...
def _image_present(client, image_uuid):
    """Check if an image is present in xmonitor.
