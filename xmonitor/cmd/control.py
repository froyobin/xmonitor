...
RELOAD_SERVERS = ['xmonitor-api', 'xmonitor-registry']
GRACEFUL_SHUTDOWN_SERVERS = ['xmonitor-api', 'xmonitor-registry',
                             'xmonitor-scrubber']
...
def pid_files(server, pid_file):
    ...
    if pid_file:
        ...
        if os.path.exists('/var/run/xmonitor/%s.pid' % server):
            pid_files = ['/var/run/xmonitor/%s.pid' % server]
...
def add_command_parsers(subparsers):
    ...
    parser.set_defaults(servers=['xmonitor-' + s for s in ALL_SERVERS])
